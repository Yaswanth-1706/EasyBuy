...
const EachProductDetails = () => {
  ...
  const tokens=localStorage.getItem('token');
    useEffect(() => {
        const fetch = async () => {
            try{
            const response = await axios.get(`https://easybuy-user.onrender.com/api/auth/products/getProductByCategory/${data.categoryName}`,{
          method: "GET",
          headers: {
...
try{
 const response=await axios.post("https://easybuy-user.onrender.com/api/auth/cart/addcart",cartData,{
          method: "GET",
          headers: {
    "Authorization": `Bearer ${tokens}`,
  }
