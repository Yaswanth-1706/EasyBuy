...
const Cart = () => {
    ...
    const user=JSON.parse(localStorage.getItem('user'))
     const fetchdata=async() => {
        try{
            const response=await axios.get(`https://easybuy-user.onrender.com/api/auth/cart/getCartById/${user.userId}`,{
          method: "GET",
          headers: {
     ...
     const fetch=async()=>{
      ...
      data.map(async (item,key) => {
        const response = await axios.get(
            `https://easybuy-user.onrender.com/api/auth/products/getProduct/${item.productId}`,{
          method: "GET",
          headers: {
    "Authorization": `Bearer ${tokens}`,
    "Content-Type": "application/json"
    ...
    useEffect(()=>{
     ...
     const removeItem= async(id)=>{
        ...
        try{
        const response=await axios.delete(`https://easybuy-user.onrender.com/api/auth/cart/deletecart/${deleteId}`,{
          method: "DELETE",
          headers: {
    "Authorization": `Bearer ${tokens}`,
    "Content-Type": "application/json"
